...
def average_models(fast_model, slow_model, straggler_ratio):
    total_weight = 1.0
    fast_model_weight = total_weight - straggler_ratio
    slow_model_weight = straggler_ratio
    averaged_state_dict = {}
    for key in fast_model.keys():
        averaged_state_dict[key] = (fast_model_weight * fast_model[key] + slow_model_weight * slow_model[key]) / total_weight
    
    return averaged_state_dict



def  train_centralized(fast_models,stragglers_idx, global_model):
    ...
    for global_epoch in range(0, args.global_epochs):
        ...
        if (global_epoch+1) % SYNC == 0:
            ...
            try:
                ...
                global_model_fast.load_state_dict(average_models(fast_model=global_model_fast.state_dict(),
                                                                 slow_model=torch.load(latest_dfl_model),
                                                                 straggler_ratio=args.stragglers_percent))
            except:
                textio.cprint(f"error in sync : {latest_dfl_model}")
...
def  train_decentralised(slow_models,stragglers_idx, global_model):
    ...
    for global_epoch in range(0, args.global_epochs):
        ...
        # utils.distribute_model(slow_models, global_model)
        users_loss = []

        for user_idx in slow_models.keys():
            # time.sleep(random.uniform(0.01,0.2)) # time between 0.01 and 0.2 seconds.
            # time.sleep(random.uniform(0.01,0.1)) # less time
            time.sleep(random.uniform(0.01,0.1)) # more time
            if (args.stragglers == 'drop') & (user_idx in stragglers_idx):
        ...
        for i, user_idx in enumerate(client_ids):
            ...
            # print(f"Round : { global_epoch} Client id : {user_idx} acc : {utils.test(val_loader, slow_models[next_user_idx]['model'], test_creterion, args.device)}")




            #fullly connected approach
            averaged_model = copy.deepcopy(global_model_slow)
            # Initialize model parameters with zeros
            for param in averaged_model.parameters():
                param.data.fill_(0)

            models = []
            for m in slow_models.keys():
                if m != user_idx:
                    models.append(slow_models[m]['model'])
            # print(f"Avergaing client : {user_idx} with {len(models)} clients ")
            
            # Sum up all models' parameters
            for model in models:
                for param, averaged_param in zip(model.parameters(), averaged_model.parameters()):
                    averaged_param.data.add_(param.data)

            # Average the parameters by dividing by the number of models
            for param in averaged_model.parameters():
                param.data.div_(len(models))
            slow_models[user_idx]['model'].load_state_dict(averaged_model.state_dict())
        global_model_slow.load_state_dict(averaged_model.state_dict())
        
        # utils.FedAvg(slow_models, global_model_slow)
        ...
        if (global_epoch+1) % SYNC == 0:
            #synchronise the models after every 10 rounds with the latest avelaible cfl model
            ...
            try:
                ...
                global_model_slow.load_state_dict(average_models(fast_model=torch.load(latest_cfl_model),
                                                                 slow_model=global_model_slow.state_dict(),
                                                                 straggler_ratio=args.stragglers_percent))
            except:
                textio.cprint(f"error in sync : {latest_cfl_model}")
...
if __name__ == '__main__':
    ...
    save_dir_1 = f"Make_logs/ResNet/FIRST_Completed/{args.dir_alpha}dir_{args.seed}Seed_{args.stragglers_percent}strg_{SYNC}sync"
    ...
    filename = f"MAK_log_{current_time}.csv"
    save_dir2 = f"Make_logs/ResNet/FIRST_Exception/{args.dir_alpha}dir_{args.seed}Seed_{args.stragglers_percent}strg_{SYNC}sync"
    ...
    # for m in local_models.keys():
    #     print(f"Client : {m} data : {len(local_models[m]['data'])}")
    # exit()
    # stragglers
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        f1 = executor.submit(train_decentralised, slow_models, stragglers_idx, global_model)
