...
# def average_models(model1, model2):
#     """ Averages two PyTorch model state_dicts using FedAvg """
#     return (utils.FedAvg(model1, model2))
...
def  train_centralized(fast_models,stragglers_idx, global_model):
    ...
    for global_epoch in range(0, args.global_epochs):
        ...
        for user_idx in fast_models.keys():
            ...
            user_loss = []
            for local_epoch in range(0, args.local_epochs): # Default is 1
...
def  train_decentralised(slow_models,stragglers_idx, global_model):
    ...
    for global_epoch in range(0, args.global_epochs):
        if stop_event.is_set():
            return 
        textio.cprint(f"++++++++++++ [DFL slow ] Round {global_epoch+1}/{args.global_epochs}")
...
if __name__ == '__main__':
    ...
    save_dir = "MAK_logs_Delay/Cifar_10/0.3/10_Rounds"
