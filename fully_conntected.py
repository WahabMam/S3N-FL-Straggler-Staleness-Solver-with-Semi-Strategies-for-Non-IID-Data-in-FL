...
def  train_centralized(fast_models,stragglers_idx, global_model):
    ...
    for global_epoch in range(0, args.global_epochs):
        if stop_event.is_set():
            return 
        textio.cprint(f"++++++++++++ [Centralized ] Round {global_epoch+1}/{args.global_epochs}")
...
def train_decentralised(slow_models, stragglers_idx, global_model):
    ...
    textio.cprint(f'Training decentralized slow models with local rounds (Fully Connected)')
    best_val_acc = np.inf
    client_ids = list(slow_models.keys())  # Get client IDs

    for global_epoch in range(0, args.global_epochs):
        print(f"Global Epochs---------------------> {global_epoch + 1}")
        if stop_event.is_set():
            return 
        textio.cprint(f"++++++++++++ [DFL slow with local rounds] Round {global_epoch + 1}/{args.global_epochs}")
        ...
        # **Fully Connected Communication Step with Averaging**
        for i, user_idx in enumerate(client_ids):
            current_model_state = slow_models[user_idx]['model'].state_dict()

            # Average with all other clients' models
            averaged_state_dict = copy.deepcopy(current_model_state)
            for other_user_idx in client_ids:
                if other_user_idx != user_idx:
                    other_model_state = slow_models[other_user_idx]['model'].state_dict()
                    # Average the models
                    averaged_state_dict = average_models(averaged_state_dict, other_model_state)

            # Update the model with averaged state
            slow_models[user_idx]['model'].load_state_dict(averaged_state_dict)

        val_acc = utils.test(val_loader, global_model_slow, test_creterion, args.device)
        train_loss_list.append(train_loss)
        val_acc_list.append(val_acc)

        # Update slow global model
...
if __name__ == '__main__':
    ...
    filename = f"Fully_Connected_log_{current_time}.csv"
    save_dir = "experiment_logs/fully_connected/LeNet_5_20_Rounds"
    ...
    num_strugglers = len(stragglers_idx)
    print(f"****************** number of straggler are:{num_strugglers} ***********************")
