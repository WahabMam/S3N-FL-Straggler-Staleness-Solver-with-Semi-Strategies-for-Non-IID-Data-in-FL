...
def  train_centralized(fast_models,stragglers_idx, global_model):
    ...
    for global_epoch in range(0, args.global_epochs):
        ...
        if (global_epoch+1) % SYNC == 0:
            #synchronise the models after every 10 rounds with the latest avelaible dfl model
...
if __name__ == '__main__':
    ...
    save_dir_1 = f"MAK2_logs/Fully/FIRST_Completed/{args.stragglers_percent}strg_{SYNC}sync"
    ...
    filename = f"MAK_log_{current_time}.csv"
    save_dir2 = f"MAK2_logs/Fully/FIRST_Exception/{args.stragglers_percent}strg_{SYNC}sync"
