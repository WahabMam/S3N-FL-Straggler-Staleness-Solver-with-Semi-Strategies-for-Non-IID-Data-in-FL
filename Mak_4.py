...
def  train_centralized(fast_models,stragglers_idx, global_model):
    ...
    for global_epoch in range(0, args.global_epochs):
        ...
        print(f"****************-------> Fast Clients cfl=> epoch: {global_epoch} | train_loss: {train_loss:.2f} | val_acc: {val_acc:.0f}% ")
    ...
    writer.writerow([global_epoch, np.round(train_loss, 2), val_acc_final_fast])
...
def  train_centralized_slow(slow_models,stragglers_idx, global_model):
    global_model_slow = copy.deepcopy(global_model)
    textio.cprint(f'Training centralised slow models')
    best_val_acc = np.inf
    for global_epoch in range(0, args.global_epochs):
        if stop_event.is_set():
            return 
        textio.cprint(f"++++++++++++ [Centralized slow ] Round {global_epoch}/{args.global_epochs}")
        utils.distribute_model(slow_models, global_model_slow)
        users_loss = []

        for user_idx in slow_models.keys():
            time.sleep(random.uniform(0.02, 0.1))
            if (args.stragglers == 'drop') & (user_idx in stragglers_idx):
                user_new_state_dict = copy.deepcopy(global_model_slow).state_dict()
                user_new_state_dict.update({})
                slow_models[user_idx]['model'].load_state_dict(user_new_state_dict)
                continue
        ...
        if (global_epoch+1) % SYNC == 0:
            ...
            textio.cprint(f"Updated current global model (slow) round {global_epoch} with  Cfl fast model : {latest_cfl_model}")
        ...
        textio.cprint(f'Slow clients => epoch: {global_epoch} | train_loss: {train_loss:.2f} | val_acc: {val_acc:.0f}%')
        print(f"****************-------> Slow clients CFL => epoch: {global_epoch} | train_loss: {train_loss:.2f} | val_acc: {val_acc:.0f}% ")
        writer2.writerow([global_epoch, np.round(train_loss, 2), val_acc])
    val_acc_final_fast = utils.test(val_loader, global_model_slow, test_creterion, args.device)


    textio.cprint(f'Slow Clients final acc => {val_acc_final_fast}')
    writer2.writerow([global_epoch, np.round(train_loss, 2), val_acc_final_fast])
    np.save(f'checkpoints/{args.exp_name}/train_loss_list_cen.npy', train_loss_list)
    np.save(f'checkpoints/{args.exp_name}/val_acc_list_cen.npy', val_acc_list)
...
def  train_decentralised(slow_models,stragglers_idx, global_model):
    ...
    writer2.writerow([global_epoch, np.round(train_loss, 2), val_acc_final_slow])
...
if __name__ == '__main__':
    ...
    save_dir_1 = f"MAK4_logs/New_logic/FIRST_Completed/{args.stragglers_percent}strg_{SYNC}_Sync"
    ...
    filename = f"MAK_log_{current_time}.csv"
    save_dir2 = f"MAK4_logs/New_logic//FIRST_Exception/{args.stragglers_percent}strg_{SYNC}_Sync"
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        f1 = executor.submit(train_centralized_slow, slow_models, stragglers_idx, global_model)
